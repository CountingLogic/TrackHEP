...
# MAX_TRACKS = 4 , already called in arguemtns 


def gen_dataset(data, lpred):
    events = data.groupby('event')
    dataset_l0 = []

    for name, ev in events:
        pl0 = ev[ev['layer'] == 0]
        pl1 = ev[ev['layer'] == lpred - 1]
        pl2 = ev[ev['layer'] == lpred]
        ...
        for track in l0_cluster:
            ...
            for l in range(1, lpred):
                y = ev[(ev['layer'] == l) & (ev['cluster_id'] == track)]['phi'].values
                tmp = distphi(y0, y)
                if tmp.size != 0:
                    yd[l - 1] = tmp[0]
                y0 = y.copy()

            if np.any(np.isin(yd, 0)):
                continue
            else:
                y = ev[(ev['layer'] == (lpred - 1)) & (ev['cluster_id'] == track)]['phi'].values
                y_true = pl2[pl2['cluster_id'] == track]['phi'].values
                
                d_true = distphi(y, y_true)
                
                if d_true.size != 0:
                    
                    x1 = distphi(y, pl1.phi.values)
                    len_x1 = x1.shape[0]
                    ...
                    dataset_l0.append(dict({'ev': name, 'track': track, 'x': x, 'y': y_true, 'yd': yd, 'x1': x1, 'x2': x2, 'dtrue': d_true}))    
    return dataset_l0

dataset_l0_train = gen_dataset(sim_data_train, LPRED)
dataset_l0_test = gen_dataset(sim_data_test, LPRED)
...
class trackDataset(utils_data.Dataset):
    ...
    def __getitem__(self, idx):
        sample = {'yd': self.layer_dataset[idx]['yd'], 
                  'x1': self.layer_dataset[idx]['x1'], 
                  'x2': self.layer_dataset[idx]['x2'], 
                  'dtrue': self.layer_dataset[idx]['dtrue']}
        return sample
#BATCH_SIZE = 50
...
def train_epoch(Xy, model, optimizer, loss_f):
    ...
    x2 = Xy['x2'].to(device)
    yd = Xy['yd'].to(device)
    ytrue = Xy['dtrue'].to(device)

    model.train()

    optimizer.zero_grad()
    
    outputs = model(x1, x2, yd)

    loss = loss_f(outputs, ytrue)
...
len_yd = LPRED - 1
track_lstm = Track_Linear(mt1+1,mt2+1, 8, len_yd ).double().to(device)
print(track_lstm)
...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(data_loader_te):
        ...
        dtrue = batch['dtrue'].to(device)

        outputs = track_lstm(x1, x2, yd)
        loss_te = loss_f(outputs, dtrue)                         
